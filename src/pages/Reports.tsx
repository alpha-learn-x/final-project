...
const Reports = () => {
  ...
  // ✅ Use current logged-in user from localStorage
  ...
  const sanitizeForFile = (s: string) =>
      (s || "User").trim().replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_-]/g, "");

  const generatePDFReport = () => {
    // Create a comprehensive PDF-like report image (PNG download)
    ...
    if (ctx) {
      ...
      ctx.fillText(`Student: ${userName}`, 400, 85);
      ...
      // Download file name uses logged-in user
      const safeUser = sanitizeForFile(userName);
      const link = document.createElement("a");
      link.download = `${safeUser}_Learning_Report_${new Date()
          .toISOString()
          .split("T")[0]}.png`;
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600">{userName}&apos;s Progress Report</p>
            </div>
            <div className="flex space-x-4">
              <Button onClick={generatePDFReport} className="bg-red-500 hover:bg-red-600 text-white">
